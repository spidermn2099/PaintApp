...
namespace PaintApp.Controls
{
    ...
    public class CanvasControl : UserControl
    {
            ...
            set
            {
                ...
                else
                {
                    try
                    {
                        ...
                        currentPen = new Pen(value.Color, w);
        ...
        {
            get => currentBrush;
            set => currentBrush = (value is SolidBrush sb) ? new SolidBrush(sb.Color) : new SolidBrush(Color.White);
        }
        ...
        public void ChangeSelectedPen(Pen pen)
        {
            if (selectedShapes.Count == 0) return;
            foreach (var s in selectedShapes)
                commandManager.ExecuteCommand(new ModifyShapeCommand(s, pen, s.Brush));
            Invalidate();
        }

        public void ChangeSelectedBrush(Brush brush)
        {
            if (selectedShapes.Count == 0) return;
            foreach (var s in selectedShapes)
                commandManager.ExecuteCommand(new ModifyShapeCommand(s, s.Pen, brush));
            Invalidate();
        }
        ...
        {
            if (selectedShapes.Count == 0) return;
            var toDel = new List<Shape>(selectedShapes);
            foreach (var l in layerManager.Layers)
                foreach (var s in toDel)
                    l.RemoveShape(s);
            commandManager.ExecuteCommand(new RemoveShapeCommand(toDel, shapes));
        ...
        public void DeleteAllShapes()
        {
            ...
            commandManager.ExecuteCommand(new RemoveAllShapesCommand(shapes, layerManager.Layers));
            selectedShapes.Clear();
            Invalidate();
        }
        ...
        {
            if (selectedShapes.Count == 0) return;
            commandManager.ExecuteCommand(new RotateShapeCommand(new List<Shape>(selectedShapes), angle));
            Invalidate();
        }
        ...
        {
            if (selectedShapes.Count == 0) return;
            commandManager.ExecuteCommand(new MoveShapeCommand(selectedShapes, dx, dy));
            Invalidate();
        }
        ...
        {
            for (int l = layerManager.Layers.Count - 1; l >= 0; l--)
            {
                if (!layerManager.Layers[l].Visible) continue;
                for (int i = layerManager.Layers[l].Shapes.Count - 1; i >= 0; i--)
                    if (layerManager.Layers[l].Shapes[i].Contains(p))
                        return layerManager.Layers[l].Shapes[i];
            }
            return null;
        }
        ...
        {
            if (new Rectangle(rect.Left - 5, rect.Top - 5, 10, 10).Contains(pos)) return 0;
            if (new Rectangle(rect.Right - 5, rect.Top - 5, 10, 10).Contains(pos)) return 1;
            if (new Rectangle(rect.Left - 5, rect.Bottom - 5, 10, 10).Contains(pos)) return 2;
            if (new Rectangle(rect.Right - 5, rect.Bottom - 5, 10, 10).Contains(pos)) return 3;
            return -1;
        }
        ...
        private void DrawRotationArrows(Graphics g, Rectangle rect)
        {
            ...
            {
                Point[] left = {
                    new Point(cx - 20, top), new Point(cx - 20, top - 8),
                    new Point(cx - 30, top - 8), new Point(cx - 30, top - 15),
                    new Point(cx - 40, top), new Point(cx - 30, top + 15),
                    new Point(cx - 30, top + 8), new Point(cx - 20, top + 8)
                };
                Point[] right = {
                    new Point(cx + 20, top), new Point(cx + 20, top - 8),
                    new Point(cx + 30, top - 8), new Point(cx + 30, top - 15),
        ...
        private bool OnLeftArrow(Point pos, Rectangle rect)
        {
            int cx = rect.Center().X, top = rect.Top - 20;
            return pos.X >= cx - 40 && pos.X <= cx - 20 && pos.Y >= top - 15 && pos.Y <= top + 15;
        }

        private bool OnRightArrow(Point pos, Rectangle rect)
        {
            int cx = rect.Center().X, top = rect.Top - 20;
            return pos.X >= cx + 20 && pos.X <= cx + 40 && pos.Y >= top - 15 && pos.Y <= top + 15;
        }
        ...
        private void CanvasControl_Paint(object sender, PaintEventArgs e)
        {
                ...
                foreach (var shape in layer.Shapes)
                {
                    if (shape.Selected)
                    {
                        ...
                        using (Brush white = new SolidBrush(Color.White))
                        {
                            ...
                            e.Graphics.FillRectangle(white, rect.Left - 4, rect.Top - 4, 8, 8);
                            e.Graphics.DrawRectangle(black, rect.Left - 4, rect.Top - 4, 8, 8);
                            e.Graphics.FillRectangle(white, rect.Right - 4, rect.Top - 4, 8, 8);
                            e.Graphics.DrawRectangle(black, rect.Right - 4, rect.Top - 4, 8, 8);
                            e.Graphics.FillRectangle(white, rect.Left - 4, rect.Bottom - 4, 8, 8);
                            e.Graphics.DrawRectangle(black, rect.Left - 4, rect.Bottom - 4, 8, 8);
                            e.Graphics.FillRectangle(white, rect.Right - 4, rect.Bottom - 4, 8, 8);
                            e.Graphics.DrawRectangle(black, rect.Right - 4, rect.Bottom - 4, 8, 8);
                        }
            ...
            {
                using (Pen dash = new Pen(Color.Blue, 1) { DashStyle = DashStyle.Dash })
                using (Brush fill = new SolidBrush(Color.FromArgb(50, 0, 0, 255)))
                {
                    e.Graphics.FillRectangle(fill, selectionRect);
                    e.Graphics.DrawRectangle(dash, selectionRect);
                }
            }
            if (isDrawing && currentShape != null)
                currentShape.DrawPreview(e.Graphics, lastPoint);
        }
                ...
                if (currentMode == DrawingMode.Select)
                {
                    ...
                    if (clicked != null)
                    {
                        if (ModifierKeys.HasFlag(Keys.Control))
                        {
                            ...
                            if (clicked.Selected) selectedShapes.Add(clicked);
                            else selectedShapes.Remove(clicked);
                        }
        ...
        private void CanvasControl_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                {
                    Point delta = new Point(e.X - resizeStartPoint.X, e.Y - resizeStartPoint.Y);
                    Rectangle orig = resizingShape.GetBoundingRect();
                    commandManager.ExecuteCommand(new ResizeShapeCommand(resizingShape, delta, resizeHandle, orig));
                    resizeStartPoint = e.Location;
                    Invalidate();
                }
                ...
                else if (currentMode == DrawingMode.Select && selectedShapes.Count > 0 && (e.Button & MouseButtons.Left) != 0)
                {
        ...
        private void CanvasControl_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                else if (isDrawing && currentShape != null && (currentMode == DrawingMode.Plugin || (currentMode != DrawingMode.Polyline && currentMode != DrawingMode.Polygon)))
                {
                    currentShape.SetEndPoint(e.Location);
                    if (layerManager.CurrentLayer != null)
                        commandManager.ExecuteCommand(new AddShapeCommand(currentShape, layerManager.CurrentLayer, shapes));
        ...
        private void CanvasControl_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.Control)
            {
                ...
                else if (e.KeyCode == Keys.C) CopySelectedShapes();
                else if (e.KeyCode == Keys.V) PasteShapes();
            }
            else if (e.KeyCode == Keys.Delete)
            {
                DeleteSelectedShapes();
            }
            else if (e.KeyCode == Keys.R && selectedShapes.Count > 0)
            {
                RotateSelectedShapes(15);
            }
            else
            {
                int dx = 0, dy = 0;
        ...
        private void CanvasControl_MouseWheel(object sender, MouseEventArgs e)
        {
            if (currentMode == DrawingMode.Select && selectedShapes.Count > 0)
            {
                if (e.Delta > 0) RotateSelectedShapes(10);
                else if (e.Delta < 0) RotateSelectedShapes(-10);
            }
        }
        ...
        private void SelectShapesInRect(Rectangle rect)
        {
            ...
            foreach (var layer in layerManager.Layers)
            {
                ...
                foreach (var shape in layer.Shapes)
                {
                    if (rect.IntersectsWith(shape.GetBoundingRect()))
                    {
                        shape.Selected = true;
